...
if(is_user_logged_in()){
    $require_feedback = cdv_get_meta("require_feedback", "", $post->ID);
    if(($require_feedback != "") && ($require_feedback != "false")){
        if(cdv_user_can_sign_circolare($current_user, $post)){
            $disabled = "";
            $has_signed = cdv_user_has_signed_circolare($current_user, $post);
            if($has_signed){
