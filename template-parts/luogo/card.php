...
$indirizzo = av_get_meta("indirizzo", '_av_luogo_', $luogo->ID);
$posizione_gps = av_get_meta("posizione_gps", '_av_luogo_', $luogo->ID);
$cap = av_get_meta("cap", '_av_luogo_', $luogo->ID);
$mail = av_get_meta("mail", '_av_luogo_', $luogo->ID);
$pec = av_get_meta("pec", '_av_luogo_', $luogo->ID);
$telefono = av_get_meta("telefono", '_av_luogo_', $luogo->ID);
...
if($luogo->post_parent == 0){
	...
	if(!$indirizzo)	$indirizzo = av_get_meta("indirizzo", "_av_luogo_", $luogo->post_parent);
	if(!$posizione_gps["lat"] || !$posizione_gps["lng"]) $posizione_gps = av_get_meta("posizione_gps", "_av_luogo_", $luogo->post_parent);
	if(!$cap) $cap = av_get_meta("cap", "_av_luogo_", $luogo->post_parent);
	if(!$mail) $mail = av_get_meta("mail", "_av_luogo_", $luogo->post_parent);
	if(!$pec) $pec = av_get_meta("pec", "_av_luogo_", $luogo->post_parent);
	if(!$telefono) $telefono = av_get_meta("telefono", "_av_luogo_", $luogo->post_parent);
}

$orario_pubblico = av_get_meta("orario_pubblico", '_av_luogo_', $luogo->ID);

if(isset($struttura->ID) && av_get_meta("telefono", '_av_struttura_', $struttura->ID) != "")
	$telefono = av_get_meta("telefono", '_av_struttura_', $struttura->ID);

if(isset($struttura->ID) && av_get_meta("mail", '_av_struttura_', $struttura->ID) != "")
	$mail = av_get_meta("mail", '_av_struttura_', $struttura->ID);

if(isset($struttura->ID) && av_get_meta("pec", '_av_struttura_', $struttura->ID) != "")
$pec = av_get_meta("pec", '_av_struttura_', $struttura->ID);

if(isset($struttura->ID) && av_get_meta("pec", '_av_struttura_', $struttura->ID) != "")
	$pec = av_get_meta("pec", '_av_struttura_', $struttura->ID);

if(isset($struttura->ID)){
	$arr_persone = av_get_meta("persone", '_av_struttura_', $struttura->ID);
	$persone = "";
	if(is_array($arr_persone)){
		foreach ($arr_persone as $id_persona){
			$persone .= av_get_display_name( $id_persona)." ";
		}
    }
